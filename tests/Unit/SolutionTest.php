...
class SolutionTest extends TestCase
{
    ...
    /**
     * Diet 솔루션 조회 테스트
     * 추천 솔루션은 Collection 객체이어야 하고, Diet 전체 솔루션 크기보다 작거나 같아야 함
     * @return void
     */
    public function test_get_diet_solution()
    {
        $dietExpert = new DietExpert();
        $maxCount = count($dietExpert::SOLUTION_LIST);

        $lifeStyles = [LifeStyleEnum::ENOUGH_MONEY->value, LifeStyleEnum::ENOUGH_TIME->value, LifeStyleEnum::STRONG_WILL->value];

        $recommendSolution = $dietExpert->getRecommendSolutions($lifeStyles);
